...
def plot_thrust_drag_model(data, kw=-2.5e-6, kx=-6e-5, ky=-6e-5):
    fig, axs = plt.subplots(1, 3, figsize=(10, 10), sharex=True, sharey=True)
    # THRUST MODEL ------------------------------------------------------------------------------
    axs[0].plot(data['t'], data['az'], label='az')
    axs[0].plot(data['t'], kw*(data['omega[0]']**2 + data['omega[1]']**2 + data['omega[2]']**2 + data['omega[3]']**2), label='T model')
    axs[0].set_xlabel('t [s]')
    axs[0].set_ylabel('acc [m/s^2]')
    axs[0].legend()
    axs[0].set_title('Thrust model: az = kw*sum(omega_i**2) \n kw = {:.2e}'.format(kw))
    
    # DRAG MODEL X ------------------------------------------------------------------------------
    axs[1].plot(data['t'], data['ax'], label='ax')
    axs[1].plot(data['t'], kx*data['vbx']*(data['omega[0]']+data['omega[1]']+data['omega[2]']+data['omega[3]']), label='Dx model')
    axs[1].set_xlabel('t [s]')
    axs[1].set_ylabel('acc [m/s^2]')
    axs[1].legend()
    axs[1].set_title('Drag model X: ax = kx*vbx*sum(omega_i) \n kx = {:.2e}'.format(kx))
    
    # DRAG MODEL Y ------------------------------------------------------------------------------
    axs[2].plot(data['t'], data['ay'], label='ay')
    axs[2].plot(data['t'], ky*data['vby']*(data['omega[0]']+data['omega[1]']+data['omega[2]']+data['omega[3]']), label='Dy model')
    axs[2].set_xlabel('t [s]')
    axs[2].set_ylabel('acc [m/s^2]')
    axs[2].legend()
    axs[2].set_title('Drag model Y: ay = ky*vby*sum(omega_i) \n ky = {:.2e}'.format(ky))
