
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// This file is automatically generated in "Generate Kalman Filter C Code.ipynb" from https://github.com/tudelft/kalman_filter   //
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

#ifndef EKF_CALC_H
#define EKF_CALC_H

#include <stdint.h>

#define N_STATES 16
#define N_INPUTS 6
#define N_PROC_NOISES 12

// 2 measurement models: mocap, vbody
#define N_MEASUREMENTS_MOCAP 7
#define N_MEASUREMENTS_VBODY 2

// getters
float* ekf_get_X(void);     // get state vector
float* ekf_get_P(void);     // get covariance matrix (lower diagonal)

#define ekf_P_index(i,j) ((i>=j) ? ekf_get_P()[i*(i+1)/2+j] : ekf_get_P()[j*(j+1)/2+i])
#define ekf_X_index(i) ekf_get_X()[i]

// setters
void ekf_set_Q(float Q[N_INPUTS]);                    // set process noise covariance matrix diagonal
// 2 measurement models: mocap, vbody
void ekf_set_R_mocap(float R[N_MEASUREMENTS_MOCAP]);              // set measurement noise covariance matrix diagonal
void ekf_set_R_vbody(float R[N_MEASUREMENTS_VBODY]);              // set measurement noise covariance matrix diagonal
void ekf_set_X(float X0[N_STATES]);                   // set state vector
void ekf_set_P_diag(float P_diag[N_STATES]);          // set covariance matrix diagonal

// prediction and update functions
void ekf_predict(float U[N_INPUTS], float dt);

// 2 measurement models: mocap, vbody
void ekf_update_mocap(float Z[N_MEASUREMENTS_MOCAP]);
void ekf_update_vbody(float Z[N_MEASUREMENTS_VBODY]);

#endif // EKF_CALC_H
